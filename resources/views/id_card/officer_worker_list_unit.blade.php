...
<div class="modal fade" id="worker_photo_link" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Worker & Unit photo link</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p style="margin-bottom: 0 !important;"><b>Unit photo link copy:</b></p>
                <span style="background: #000;color: #fff;padding: 0 8px" id="unit_photo_link_set"></span>
...
@section('additionalJS')
    ...
    <script>
        $(document).ready(function() {
            $('#officer_worker_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url:"{{ route('all.officer.worker.unit') }}",
                },
                columns:[
    ...
    <script>
        ...
        function worker_photo_link(id) {
            $.ajax({
                method: "GET",
                url: "{{route('unit.worker.photo.link')}}",
                data: {id:id}
            }).done(function (data) {

                var worker_photo='{{asset('/')}}'+data.worker_photo;
                var unit_photo='{{asset('/')}}'+data.unit.unit_photo;

                $('#unit_photo_link_set').html(unit_photo);
